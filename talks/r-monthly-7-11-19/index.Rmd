...
date: "Slides and code **[here](https://almplatform/tfs/UnmanagedCollection/Shared%20Analytical%20Code/_git/boeCharts?path=%2Ftalks%2Fr-monthly-7-11-19&version=GBmaster&_a=contents)**"
output:
...
- Advocates for a "declarative" visualisation workflow - you describe the relationships between your data's variables and chart aesthetics, it takes care of the finer details (if you don't want to)
...
- A bridge between the Bank's visual identity guidelines and reproducible, publication-quality data visualisation 🖼️
...
.font120[Alastair Firrell, Joshua Allen, Me, ?]
...
```{r large-plot, eval=FALSE, echo=FALSE}
p <- ggplot(data = big_mac_index) +
  aes(x = date, y = USD,
      colour = currency_code) +
  # add line at zero
  geom_hline(yintercept = 0, colour = boe_neutral$dark_grey,
             size = 0.75, linetype = "dashed") +
  # add series lines
  geom_line(size = 1.5, key_glyph = "timeseries") +
  # addmarkers for latest points
  geom_point(data = filter(big_mac_index, date == max(date)),
             size = 5, show.legend = FALSE) +
  # apply boe palette
  scale_color_discrete_boe(
    palette = "vibrant_c",
    reverse = TRUE) +
  # edit date axis settings
  scale_x_date(expand = expand_scale(mult = c(0, 0.01)), date_breaks = "1 year", date_labels = "%Y") +
  scale_y_continuous(labels = scales::percent_format(accuracy = 5L), position = "right") +
  # add chart labels
  labs(
    title = "Big Macs are cheaper in the UK!",
    subtitle = "Index of Mcdonald's Big Mac prices relative to the US dollar. Prices adjusted for GDP.",
    y = "Index (relative to USD)", x = NULL,
    caption = "Source: https://github.com/TheEconomist/big-mac-data"
  ) +
  # add theme
  theme_overground(caption_face = "italic", caption_size = 14,
                   plot_margin = c(10, 10, 10, 20)) +
  theme(legend.title =  element_blank(), legend.position = "top")

# left align titles
p <- left_align_titles(p)
