#' @title A \code{ggplot2} theme for website-ready charts
#'
#' @description Provides a theme to produce
#' Bank website style visualisations in \code{ggplot2}.
#' 
#' @examples
#'
#' library(ggplot2)
...
theme_mcg_pub <- function(base_size = 12, base_colour = "#2b2b2b",
  ...
  mcg_theme <- theme_classic(base_size = base_size)
  
  # grid
  if (inherits(grid, "character") | grid == TRUE) {
    
    mcg_theme <- mcg_theme + theme(panel.grid=element_line(color=grid_col, size=0.2))
    mcg_theme <- mcg_theme + theme(panel.grid.major=element_line(color=grid_col, size=0.2))
    mcg_theme <- mcg_theme + theme(panel.grid.minor=element_line(color=grid_col, size=0.15))
    
    if (inherits(grid, "character")) {
      if (regexpr("X", grid)[1] < 0) mcg_theme <- mcg_theme + theme(panel.grid.major.x=element_blank())
      if (regexpr("Y", grid)[1] < 0) mcg_theme <- mcg_theme + theme(panel.grid.major.y=element_blank())
      if (regexpr("x", grid)[1] < 0) mcg_theme <- mcg_theme + theme(panel.grid.minor.x=element_blank())
      if (regexpr("y", grid)[1] < 0) mcg_theme <- mcg_theme + theme(panel.grid.minor.y=element_blank())
    }
