...
The helper function `left_align_titles` can be used to fix titles to the left of the chart window (as is the case in *Bank Overground* publications). Note that 
...
`theme_inflation_report` approximates the design of charts in [the Inflation Report](https://www.bankofengland.co.uk/inflation-report/2018/august-2018/the-labour-market-and-pay). Note the arguments passed to `scale_y_continuous()` to achieve the correct y-axis format (as well as `scale_colour_discrete_boe`, seen earlier, to do the colours).
